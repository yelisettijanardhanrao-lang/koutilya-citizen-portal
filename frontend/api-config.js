(function(){
  ...
  const LOCATION_DATA_URL = 'https://raw.githubusercontent.com/mchittineni/india-village-finder/main/andhra_pradesh/data/andhra_pradesh_villages.csv';
  window.KSPL_API_BASE = API_BASE;
  window.KSPL_LOCATION_DATA_URL = LOCATION_DATA_URL;
  const nativeFetch = window.fetch.bind(window);
  window.fetch = function(input, init){
    let url = typeof input === 'string' ? input : (input && input.url) || '';
    let isApi = false;
    if (typeof url === 'string') {
      if (url === '/locations.csv' || url.startsWith('/locations.csv?')) {
        return nativeFetch(LOCATION_DATA_URL, init);
      }
