...
secret=String(process.env.CASHFREE_SECRET_KEY||process.env.CASHFREE_CLIENT_SECRET||'').trim();if(!appId||!secret)return res.status(503).json({success:false,message:'Cashfree is not configured.'});
const base=String(process.env.CASHFREE_ENV||'sandbox').trim().toLowerCase()==='production'
  ? 'https://api.cashfree.com/pg'
